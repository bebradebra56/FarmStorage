...
class FarmStorageVi(
    private val farmStorageContext: Context,
    private val farmStorageCallback: FarmStorageCallBack,
    private val farmStorageWindow: Window
) : WebView(farmStorageContext) {
    private var farmStorageFileChooserHandler: ((ValueCallback<Array<Uri>>?) -> Unit)? = null
    fun farmStorageSetFileChooserHandler(handler: (ValueCallback<Array<Uri>>?) -> Unit) {
        this.farmStorageFileChooserHandler = handler
    }
    ...
    init {
        ...
        webSettings.apply {
            ...
            userAgentString =
                WebSettings.getDefaultUserAgent(farmStorageContext).replace("; wv)", "")
                    .replace("Version/4.0 ", "")
        ...
        super.setWebViewClient(object : WebViewClient() {
            override fun shouldOverrideUrlLoading(
                ...
                return if (request?.isRedirect == true) {
                    ...
                    try {
                        farmStorageContext.startActivity(intent)
                    } catch (e: Exception) {
                        Toast.makeText(
                            farmStorageContext,
            ...
            override fun onPageFinished(view: WebView?, url: String?) {
                ...
                if (url?.contains("ninecasino") == true) {
                    FarmStorageApplication.farmStorageInputMode =
                        WindowManager.LayoutParams.SOFT_INPUT_ADJUST_PAN
                    Log.d(
                        FarmStorageApplication.FARM_STORAGE_MAIN_TAG,
                        "onPageFinished : ${FarmStorageApplication.farmStorageInputMode}"
                    )
                    farmStorageWindow.setSoftInputMode(WindowManager.LayoutParams.SOFT_INPUT_ADJUST_PAN);
                    farmStorageEnableStableInputScroll()
                } else {
                    FarmStorageApplication.farmStorageInputMode =
                    ...
                    farmStorageWindow.setSoftInputMode(WindowManager.LayoutParams.SOFT_INPUT_ADJUST_RESIZE);
                }
            }
        ...
        super.setWebChromeClient(object : WebChromeClient() {
            ...
            override fun onShowFileChooser(
                ...
                farmStorageFileChooserHandler?.invoke(filePathCallback)
                return true
            }
            ...
            override fun onCreateWindow(
                ...
                farmStorageHandleCreateWebWindowRequest(resultMsg)
                return true
            }
        })
    }
    ...
    private fun farmStorageEnableStableInputScroll() {
        evaluateJavascript(
            """
        (function() {
    ...
    fun farmStorageFLoad(link: String) {
        super.loadUrl(link)
    }

    private fun farmStorageHandleCreateWebWindowRequest(resultMsg: Message?) {
        if (resultMsg == null) return
        if (resultMsg.obj != null && resultMsg.obj is WebView.WebViewTransport) {
            val transport = resultMsg.obj as WebView.WebViewTransport
            val windowWebView =
                FarmStorageVi(farmStorageContext, farmStorageCallback, farmStorageWindow)
            transport.webView = windowWebView
            resultMsg.sendToTarget()
            farmStorageCallback.farmStorageHandleCreateWebWindowRequest(windowWebView)
        }
    }
